...
def dilation_wrap(x, kernel, stride=[1, 1], rates=[1, 1], padding=[0, 0]):
    ...
    if x.device == torch.device("cpu"):
        ...
        # CUDA implementation
        output = dilation2d_cuda.dilation2d(
            x,
            kernel,
            stride[0],
            stride[1],
...
def single_obj_scoremap(mask, filter_size=21):
    ...
    objectmap_list = []
    kernel_dil = torch.ones(filter_size, filter_size, device=mask.device) / float(
        filter_size * filter_size
    )
    print("s shape {}".format(s))

    for i in range(s[0]):
        # create initial object map
        objectmap = max_loc[i].clone()

        num_passes = max(s[2], s[3]) // (filter_size // 2)  # 48

        st = time.time()
        for j in range(num_passes):
            objectmap = torch.reshape(objectmap, (1, s[2], s[3]))
            objectmap_dil = dilation_wrap(
                objectmap, kernel_dil, padding=[padding_size, padding_size]
            )
            objectmap_dil = torch.reshape(objectmap_dil, [s[2], s[3]])
            objectmap = torch.round(detmap_fg[i] * objectmap_dil)
        print("for loop time {}".format(time.time() - st))
...
def calc_center_bb(binary_class_mask):
    ...
    for i in range(s[0]):
        if len(binary_class_mask[i].nonzero().shape) < 2:
            bb = torch.zeros(2, 2, dtype=torch.int32, device=binary_class_mask.device)
            bbs.append(bb)
            centers.append(
                torch.tensor([160, 160], dtype=torch.int32, device=binary_class_mask.device)
            )
...
def plot_hand(coords_hw, axis, color_fixed=None, linewidth="1"):
    ...
    colors = np.array(
        [
            ...
            [0.22454143, 1.0, 0.74320051],
            [0.40164453, 1.0, 0.56609741],
            [0.56609741, 1.0, 0.40164453],
            [0.74320051, 1.0, 0.22454143],
            [0.90765338, 1.0, 0.06008855],
            [1.0, 0.82861293, 0.0],
            [1.0, 0.63979666, 0.0],
            [1.0, 0.43645606, 0.0],
            [1.0, 0.2476398, 0.0],
            [0.96345811, 0.0442992, 0.0],
            [0.73172906, 0.0, 0.0],
            [0.5, 0.0, 0.0],
        ]
    )
